...
struct Params {
    // Corrected bedroom dimensions: 505 cm x 505 cm
    double Lx = 5.05;
    double Ly = 5.05;
    ...
    double D = 0.0018164727813346864;
    double h = 0.11026733442621521;
    double h_w = 0.11026733442621521;
    ...
    double Smax = 5.766483445358527;

    int kmax = 100;
    double TOL = 1e-8;

    // Sensor location used for optional point comparison.
    // Keep/check this against real bedroom sensor position if available.
    int ic;
    int jc;
...
void apply_all_boundaries(const Params& p, Grid& T) {
    ...
    // NORTH (y=Ny-1) stays at room temperature
    for (int i = 1; i < Nx - 1; ++i) {
        T[i][Ny - 1] = Troom;
    }

    // ==================== EXTERNAL WALLS ====================
    // EAST wall: convective heat loss
    for (int j = 1; j < Ny - 1; ++j) {
        T[Nx - 1][j] = A * Tinf + B * T[Nx - 2][j];
    }
...
void init_source(const Params& p, Grid& S) {
    ...
    // Update these coordinates if you know the exact heater position in the corrected bedroom plan.
    double hx = 1.0;
    double hy = 4.3;
...
int main() {
    std::filesystem::create_directory("bedroom");
    ...
    CN_algorithm("bedroom", p, save_times);

    std::cout << "Bedroom simulation completed.\n";
    std::cout << "Grid: " << p.Nx << " x " << p.Ny << "\n";
    std::cout << "Output: bedroom/T_*.dat and bedroom/average_temperature.dat\n";
